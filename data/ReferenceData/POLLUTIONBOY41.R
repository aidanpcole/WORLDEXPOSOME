...
states2011 <- st_read("/Users/Aidan/Desktop/Raster-Timelapse/Raster-Timelapse/data/Boundaries/indiastates2011.geojson") %>%
  mutate(State = ST_NAME,
         "State/Union Territory" = "State") %>%
  dplyr::select(State,"State/Union Territory",geometry)
...
Census2011 <- read.csv("/Users/Aidan/Desktop/Raster-Timelapse/Raster-Timelapse/data/ReferenceData/india-districts-census-2011.csv") %>%
  ...
  dplyr::select("State",Population,Male,Literate,Workers,Housholds_with_Electric_Lighting,Households_with_Internet,Households_with_Computer,Households,Rural_Households,Urban_Households,Below_Primary_Education,Primary_Education,Middle_Education,Secondary_Education,Higher_Education,Graduate_Education,Other_Education,Total_Education,Age_Group_0_29,Age_Group_30_49,Age_Group_50,Households_with_Car_Jeep_Van,Households_with_Scooter_Motorcycle_Moped,Households_with_Television,Households_with_Telephone_Mobile_Phone,Ownership_Owned_Households,Ownership_Rented_Households,Household_size_1_to_2_persons,Household_size_3_to_5_persons_Households,Household_size_6_8_persons_Households,Household_size_9_persons_and_above_Households,Total_Power_Parity)
...
CleanCensus2011$State <- str_replace(CleanCensus2011$State, "Nct Of Delhi", "Delhi")

x <- unique(CleanCensus2011$State)
y <- unique(StatesandUTs2011$State)




### Currently 10 variables, can add more if fit in bind pop ups on leaflet
CensusStates2011 <- merge(CleanCensus2011, StatesandUTs2011, by = "State") %>%
  group_by(State) %>%
  summarise("Total Population" = sum(Population),
            "Total Males" = sum(Male),
            "Total Literate" = sum(Literate),
            "Total Workers" = sum(Workers),
            "Total Households with Internet" = sum(Households_with_Internet),
            "Total Households with Computer" = sum(Households_with_Computer),
            ...
            "Total 50 Years or Older" = sum(Age_Group_50),
            "Total Households with Transportation" = (sum(Households_with_Car_Jeep_Van) + sum(Households_with_Scooter_Motorcycle_Moped)),
            "Total Households with Television" = sum(Households_with_Television),
            "Total Households with Phone" = sum(Households_with_Telephone_Mobile_Phone),
            "Total Households Owned" = sum(Ownership_Owned_Households),
            "Total Households Rented" = sum(Ownership_Rented_Households),
            "Total Households with 1-5 People" = (sum(Household_size_1_to_2_persons) + sum(Household_size_3_to_5_persons_Households)),
            "Total Households with >6 People" = (sum(Household_size_6_8_persons_Households) + sum(Household_size_9_persons_and_above_Households)),
            "Total Purchasing Power" = sum(Total_Power_Parity)) %>%
  mutate("Percent Male" = round((CensusStates2011$`Total Males`/CensusStates2011$`Total Population`) * 100, 2) ,
         "Percent Literate" = round((CensusStates2011$`Total Literate`/CensusStates2011$`Total Population`) * 100, 2),
         "Percent Working" = round((CensusStates2011$`Total Workers`/CensusStates2011$`Total Population`) * 100, 2),
         "Percent w/ Internet" = round((CensusStates2011$`Total Households with Internet`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent w/ Computer" = round((CensusStates2011$`Total Households with Computer`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent Rural" = round((CensusStates2011$`Total Rural Households`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent Urban" = round((CensusStates2011$`Total Urban Households`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent w/ No Education" = round(((CensusStates2011$`Total Population` - CensusStates2011$`Total with Education`)/CensusStates2011$`Total Population`) * 100, 2),
         "Percent w/ Higher Ed" = round(((CensusStates2011$`Total with Higher Education` + CensusStates2011$`Total with Graduate Education`)/CensusStates2011$`Total with Education`) * 100, 2),
         "Percent Over 50" = round((CensusStates2011$`Total 50 Years or Older`/CensusStates2011$`Total Population`) * 100, 2),
         "Percent w/ Transportation" = round((CensusStates2011$`Total Households with Transportation`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent w/ Phone" = round((CensusStates2011$`Total Households with Phone`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent w/ TV" = round((CensusStates2011$`Total Households with Television`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent Owned Households" = round((CensusStates2011$`Total Households Owned`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent Rented Households" = round((CensusStates2011$`Total Households Rented`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent 1-5 Person Households" = round((CensusStates2011$`Total Households with 1-5 People`/CensusStates2011$`Total Households`) * 100, 2),
         "Percent >6 Person Households" = round((CensusStates2011$`Total Households with >6 People`/CensusStates2011$`Total Households`) * 100, 2),
         "Purchasing Power Quintile" = ntile(CensusStates2011$`Total Purchasing Power`,5)) %>%
  dplyr::select("State", "Total Population", "Percent Male", "Percent Literate", "Percent Working", "Percent Rural", "Percent w/ No Education", "Percent Over 50")


JUSTGEO <- CleanStates2011 %>%
  dplyr::select(State,geometry)


CompleteCensusStates2011 <- merge(CensusStates2011, JUSTGEO, by = "State")


st_write(CompleteCensusStates2011, "/Users/Aidan/Desktop/Raster-Timelapse/Raster-Timelapse/data/DataForMap/censusstates2011.geojson")
